...
func (p *program) calcTotalWeight() int {
	p.totalWeight = p.weight
	for _, child := range p.children {
		p.totalWeight += child.calcTotalWeight()
	}
...
func printNode(p *program) {
	...
	fmt.Printf("%p:%s  (%d)(%d)  %p:%v  %v\n", p, p.name, p.weight, p.totalWeight, p.parent, prntName, p.children)
}
