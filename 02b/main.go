...
func combination(row []int, startAt int) [][]int {
	var pair []int
	pairs := [][]int{}
	for i := startAt + 1; i < len(row); i++ {
		// canonical pair: [0] is the larger dividend, [1] is the smaller divisor
		if row[startAt] > row[i] {
			pair = []int{row[startAt], row[i]}
		} else {
			pair = []int{row[i], row[startAt]}
		}
		pairs = append(pairs, pair)
	}
	if startAt < len(row) {
		recursePairs := combination(row, startAt+1)
		for _, p := range recursePairs {
